...
describe('Fuzzy c-Means', function() {
    ...
    it('should always return the same clusters if we run the algorithm multiple times', function() {
        ...
        for (var i = 0; i < 10; i++) {
            ...
            if (clusters[0][0].id() != clusters2[0][0].id()) {
                expect(clusters[0]).to.deep.equal(clusters2[1]);
                expect(clusters[1]).to.deep.equal(clusters2[0]);
            }
            else {
                expect(clusters[0]).to.deep.equal(clusters2[0]);
                expect(clusters[1]).to.deep.equal(clusters2[1]);
            }
        }
    });
